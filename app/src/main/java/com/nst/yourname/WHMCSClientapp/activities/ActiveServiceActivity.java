...
public class ActiveServiceActivity extends AppCompatActivity implements AllServiceApiCallBack {
    Context context;
    @BindView(R.id.date)
    TextView date;
    ...
    public void onCreate(Bundle bundle) {
        ...
        setContentView((int) R.layout.activity_active_service);
        ButterKnife.bind(this);
        this.context = this;
        this.tv_title.setText(getResources().getString(R.string.my_series_my_active_service));
    ...
    class CountDownRunner implements Runnable {
        ...
        public void run() {
            while (!Thread.currentThread().isInterrupted()) {
                try {
                    ActiveServiceActivity.this.doWork();
                    Thread.sleep(1000);
    ...
    public void doWork() {
        runOnUiThread(new Runnable() {
            /* class com.nst.yourname.WHMCSClientapp.activities.ActiveServiceActivity.AnonymousClass1 */

            public void run() {
                try {
                    String date = Calendar.getInstance().getTime().toString();
                    String time = Utils.getTime(ActiveServiceActivity.this.context);
                    String date2 = Utils.getDate(date);
                    if (ActiveServiceActivity.this.time != null) {
                        ActiveServiceActivity.this.time.setText(time);
                    }
                    if (ActiveServiceActivity.this.date != null) {
                        ActiveServiceActivity.this.date.setText(date2);
                    }
