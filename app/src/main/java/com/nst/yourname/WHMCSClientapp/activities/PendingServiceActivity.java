...
public class PendingServiceActivity extends AppCompatActivity implements AllServiceApiCallBack {
    Context context;
    @BindView(R.id.date)
    TextView date;
    ...
    public void onCreate(Bundle bundle) {
        ...
        setContentView((int) R.layout.activity_pending_service);
        ButterKnife.bind(this);
        this.tv_title.setText(getResources().getString(R.string.my_series_my_pending_service));
        this.context = this;
        new Thread(new CountDownRunner()).start();
        this.recyclerView.setLayoutManager(new GridLayoutManager(this, 2));
        new CommanApiHitClass(this, this.context, "Pending").HitAllServicesApi();
    }
    ...
    class CountDownRunner implements Runnable {
        ...
        public void run() {
            while (!Thread.currentThread().isInterrupted()) {
                try {
                    PendingServiceActivity.this.doWork();
                    Thread.sleep(1000);
    ...
    public void doWork() {
        runOnUiThread(new Runnable() {
            /* class com.nst.yourname.WHMCSClientapp.activities.PendingServiceActivity.AnonymousClass1 */

            public void run() {
                try {
                    String date = Calendar.getInstance().getTime().toString();
                    String time = Utils.getTime(PendingServiceActivity.this.context);
                    String date2 = Utils.getDate(date);
                    if (PendingServiceActivity.this.time != null) {
                        PendingServiceActivity.this.time.setText(time);
                    }
                    if (PendingServiceActivity.this.date != null) {
                        PendingServiceActivity.this.date.setText(date2);
                    }
