...
public class SuspendedServiceActivity extends AppCompatActivity implements AllServiceApiCallBack {
    Context context;
    @BindView(R.id.date)
    TextView date;
    ...
    public void onCreate(Bundle bundle) {
        ...
        setContentView((int) R.layout.activity_suspended_service);
        ButterKnife.bind(this);
        this.tv_title.setText(getResources().getString(R.string.my_series_my_suspended_service));
        this.context = this;
        new Thread(new CountDownRunner()).start();
        this.recyclerView.setLayoutManager(new GridLayoutManager(this, 2));
        new CommanApiHitClass(this, this.context, "Suspended").HitAllServicesApi();
    }
    ...
    class CountDownRunner implements Runnable {
        ...
        public void run() {
            while (!Thread.currentThread().isInterrupted()) {
                try {
                    SuspendedServiceActivity.this.doWork();
                    Thread.sleep(1000);
    ...
    public void doWork() {
        runOnUiThread(new Runnable() {
            /* class com.nst.yourname.WHMCSClientapp.activities.SuspendedServiceActivity.AnonymousClass1 */

            public void run() {
                try {
                    String date = Calendar.getInstance().getTime().toString();
                    String time = Utils.getTime(SuspendedServiceActivity.this.context);
                    String date2 = Utils.getDate(date);
                    if (SuspendedServiceActivity.this.time != null) {
                        SuspendedServiceActivity.this.time.setText(time);
                    }
                    if (SuspendedServiceActivity.this.date != null) {
                        SuspendedServiceActivity.this.date.setText(date2);
                    }
