...
public class FraudServiceActivity extends AppCompatActivity implements AllServiceApiCallBack {
    Context context;
    @BindView(R.id.date)
    TextView date;
    ...
    public void onCreate(Bundle bundle) {
        ...
        setContentView((int) R.layout.activity_fraud_service);
        ButterKnife.bind(this);
        this.tv_title.setText(getResources().getString(R.string.my_series_my_fraud_service));
        this.context = this;
        new Thread(new CountDownRunner()).start();
        this.recyclerView.setLayoutManager(new GridLayoutManager(this, 2));
        new CommanApiHitClass(this, this.context, "Fraud").HitAllServicesApi();
    }
    ...
    class CountDownRunner implements Runnable {
        ...
        public void run() {
            while (!Thread.currentThread().isInterrupted()) {
                try {
                    FraudServiceActivity.this.doWork();
                    Thread.sleep(1000);
    ...
    public void doWork() {
        runOnUiThread(new Runnable() {
            /* class com.nst.yourname.WHMCSClientapp.activities.FraudServiceActivity.AnonymousClass1 */

            public void run() {
                try {
                    String date = Calendar.getInstance().getTime().toString();
                    String time = Utils.getTime(FraudServiceActivity.this.context);
                    String date2 = Utils.getDate(date);
                    if (FraudServiceActivity.this.time != null) {
                        FraudServiceActivity.this.time.setText(time);
                    }
                    if (FraudServiceActivity.this.date != null) {
                        FraudServiceActivity.this.date.setText(date2);
                    }
